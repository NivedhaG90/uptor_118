...
#There are 2 types of modal - overfitting and underfitting. If it reads data completely, overfitting. If didnt read properly, underfitting.
# Accuracy 1 - overfitting. Accuracy 0 - underfitting. Always our data should be between. Hence we go with regularisation to keep the data moderate.
#Penalty is what we call as regularisation
...
#Below code is to convert as dataframe which is not mandatory - start
col_names = my_data.feature_names

dataframe_input = pd.DataFrame(x, columns=col_names)
print(dataframe_input)

dataframe_output = pd.DataFrame(y,columns=["target_class"])
print(dataframe_output)
#dataframe conversion - end
